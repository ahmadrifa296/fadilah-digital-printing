<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                ...
                <p class="page-subtitle">Isi detail produk yang akan ditambahkan ke katalog</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-4xl mx-auto py-2">
        <form action="{{ route('produk.store') }}" method="POST" enctype="multipart/form-data"
              id="product-form"
              x-data="{ 
                  productType: '{{ old('product_type', 'custom') }}', 
                  calculationType: '{{ old('calculation_type', 'fixed') }}',
                    ...
                    <div class="card p-5 bg-white border border-slate-200 rounded-2xl shadow-sm">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="grid grid-cols-2 gap-4" x-show="!isSizeBased">
                                ...
                                <div class="form-group">
                                    <label for="stock" class="block text-xs font-bold text-slate-700 mb-1">Stok Awal</label>
                            ...
                            <div class="grid grid-cols-2 gap-4" x-show="isSizeBased" x-cloak>
                                <div class="form-group">
                                    ...
                                    <input type="number" name="price_per_square_meter" id="price_per_square_meter"
                                           value="{{ old('price_per_square_meter') }}"
                                           x-bind:required="isSizeBased" :disabled="!isSizeBased" min="0"
                            ...
                            {{-- Min/Max Order (untuk quantity, custom_size, quantity_custom_size) --}}
                            ...
                            <div class="grid grid-cols-2 gap-4 p-4 bg-slate-50 border border-slate-200/60 rounded-xl" x-show="isSizeBased" x-cloak>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="number" step="any" name="min_length" id="min_length" value="{{ old('min_length', 0.1) }}" :disabled="!isSizeBased" class="w-full text-xs border border-slate-200 rounded-xl px-3 py-1.5 mt-1 focus:outline-none focus:border-orange-500">
                                </div>
                                <div class="form-group">
                                    <label for="max_length" class="block text-[10px] font-bold text-slate-500">Panjang Maksimal</label>
                                    <input type="number" step="any" name="max_length" id="max_length" value="{{ old('max_length', 100.0) }}" :disabled="!isSizeBased" class="w-full text-xs border border-slate-200 rounded-xl px-3 py-1.5 mt-1 focus:outline-none focus:border-orange-500">
                                </div>
                                <div class="form-group">
                                    <label for="min_width" class="block text-[10px] font-bold text-slate-500">Lebar Minimal</label>
                                    <input type="number" step="any" name="min_width" id="min_width" value="{{ old('min_width', 0.1) }}" :disabled="!isSizeBased" class="w-full text-xs border border-slate-200 rounded-xl px-3 py-1.5 mt-1 focus:outline-none focus:border-orange-500">
                                </div>
                                <div class="form-group">
                                    <label for="max_width" class="block text-[10px] font-bold text-slate-500">Lebar Maksimal</label>
                                    <input type="number" step="any" name="max_width" id="max_width" value="{{ old('max_width', 100.0) }}" :disabled="!isSizeBased" class="w-full text-xs border border-slate-200 rounded-xl px-3 py-1.5 mt-1 focus:outline-none focus:border-orange-500">
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="description" id="description" rows="4"
                                          ...
                                          class="w-full text-xs border border-slate-200 rounded-xl px-4 py-2.5 focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500 resize-none">{{ old('description') }}</textarea>
                ...
                <div class="space-y-6">
                    ...
                    <div class="card p-5 bg-white border border-slate-200 rounded-2xl shadow-sm">
                        ...
                        <div class="border-2 border-dashed border-slate-300 rounded-2xl p-6 text-center hover:border-orange-400 hover:bg-orange-50/5 transition-all cursor-pointer relative"
                            ...
                            <p class="text-[11px] font-bold text-slate-600">Seret foto ke sini, atau klik untuk memilih</p>
                    ...
                    <div class="card p-5 bg-white border border-slate-200 rounded-2xl shadow-sm">
                        ...
                        <div class="flex items-center py-1">
                            <input type="checkbox" id="requires_design_file" name="requires_design_file" value="1" {{ old('requires_design_file') ? 'checked' : '' }} class="h-4 w-4 rounded border-slate-300 text-orange-650 focus:ring-orange-550 cursor-pointer">
                    ...
                    <div class="card p-5 bg-white border border-slate-200 rounded-2xl shadow-sm">
                        ...
                        <div class="space-y-3.5">
                            <div class="form-group">
                                ...
                                <input type="number" name="weight" id="weight" value="{{ old('weight', 500) }}" required min="1" class="w-full text-xs border border-slate-200 rounded-xl px-4 py-2 focus:outline-none focus:border-orange-500">
                            ...
                            <div class="grid grid-cols-3 gap-2">
                                <div class="form-group">
                                    ...
                                    <input type="number" name="length" id="length" value="{{ old('length', 10) }}" required min="1" class="w-full text-xs border border-slate-200 rounded-xl px-3 py-1.5 mt-1 focus:outline-none focus:border-orange-500">
                                </div>
                                <div class="form-group">
                                    <label for="width" class="block text-[10px] font-bold text-slate-500">Lebar (cm)</label>
                                    <input type="number" name="width" id="width" value="{{ old('width', 10) }}" required min="1" class="w-full text-xs border border-slate-200 rounded-xl px-3 py-1.5 mt-1 focus:outline-none focus:border-orange-500">
                                </div>
                                <div class="form-group">
                                    <label for="height" class="block text-[10px] font-bold text-slate-500">Tinggi (cm)</label>
                                    <input type="number" name="height" id="height" value="{{ old('height', 10) }}" required min="1" class="w-full text-xs border border-slate-200 rounded-xl px-3 py-1.5 mt-1 focus:outline-none focus:border-orange-500">
                                </div>
                            </div>
                        </div>
                    </div>
